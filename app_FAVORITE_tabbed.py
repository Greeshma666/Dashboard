...
with tab1:
    ...
    st.write("Every World Cup writes a Cinderella story. In 2026, **Cabo Verde** — a tiny island nation of 600,000 people — made their World Cup debut. They didn't just participate. Sidny Lopes Cabral scored FIFA's **Goal of the Tournament** — against Argentina.")
    
    st.write("**The Cabo Verde Effect:**")
    c1, c2, c3 = st.columns(3)
    c1.metric("🔍 Search Interest", "20×", "surge during tournament")
    c2.metric("🏝️ Population", "600K", "smaller than Memphis, TN")
    ...
    fig.update_layout(height=450, xaxis_tickangle=-55, yaxis=dict(tickvals=list(stage_order.values()), ticktext=list(stage_order.keys()), title=""))
...
with tab2:
    ...
    fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0,100])), height=450, showlegend=True)
    st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    ...
    fig.update_layout(height=350, coloraxis_showscale=False, yaxis_title="", xaxis_title="World Cup Titles")
